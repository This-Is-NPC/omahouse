...
- `/sys/class/drm`, or `$OMAHOUSE_DRM_ROOT` -- the screens. Every connector whose `status` is `connected` has a `dpms`, and that is how omahouse knows whether anybody is looking: a browser cannot be asked, because `.temp/spike-extension.md` §5 measured one answering `active` for twenty-five minutes with the monitor physically off.
