...
| `OMAHOUSE_AS` | the real uid | which account the **window** reads as — see below |

`OMAHOUSE_AS` is the last of them and the odd one: it moves which account
`omahouse-studio` reads the machine as, and it exists so that `mise run shots`
can draw both faces of the window — the operator's and the fiscalised person's
— from one login, and draw the same bytes on any machine. It grants nothing and
cannot: the window only reads, everything it reads is world readable by design,
and every write still goes out as `pkexec omahouse` under the real uid, where
polkit and the CLI's own refusals decide. The most it can produce is a window
whose every button is refused.

The first two of the roots above are also what keeps a run pointed somewhere
else from biting. A `Close` is refused unless the cgroup tree really is `/sys/fs/cgroup`,
...
`docs/guide.md` (`docs/guia.md` in Portuguese) walks somebody from an installed
omahouse to a profile that stands up. `docs/screens.md` is the inventory of every
screen the window draws, generated by `mise run shots` and checked by the gate.
